 ...
 */
export function initializeDefaultFile(lang = 'en') {
  const filename = `main.catala_${lang}`;
...
export async function loadFromUrl(url, filename) {
  try {
    ...
    if (!filename) {
      // Just extract last path segment - works for both absolute and relative URLs
      filename = url.split('/').pop()?.split('?')[0] || 'main.catala_en';
    }
